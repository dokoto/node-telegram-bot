...
Bot.prototype._multipart = function(options, callback) {
  ...
  function retry() {
    ...
    var form = req.form(),
      filename, type, stream, contentType;
...
Bot.prototype._setWebhook = function(webhook) {
  var self = this;
  var url = this.base_url + 'bot' + this.token + '/setWebhook' + "?" + qs.stringify({
    url: webhook
  });
        ...
        if (body.ok) {
          body.result.forEach(function(msg) {
            if (msg.update_id >= self.offset) {
              ...
              let message = (!msg.message && msg.edited_message) ? msg.edited_message : msg.message;
...
//edit By SMHassanAlavi ->editMessageText & answerCallbackQuery
...
Bot.prototype.answerCallbackQuery = function(options, callback) {
  var self = this,
    deferred = Q.defer();
